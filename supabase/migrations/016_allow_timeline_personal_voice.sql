-- A user can answer another person's timeline request without having an
-- active morning request of their own. The original function required the
-- sender's request to be open, which made valid timeline sends fail after the
-- sender had already completed their own alarm.
...
begin
  ...
  -- The receiver's request may already be voice_assigned because a
  -- Community Voice was selected. A Personal Voice sent before the alarm is
  -- still allowed and takes precedence during alarm synchronization.
  ...
  -- p_sender_morning_request_id remains in the signature for compatibility
  -- with existing clients, but the sender no longer needs an active request.
  if not exists (
    select 1
