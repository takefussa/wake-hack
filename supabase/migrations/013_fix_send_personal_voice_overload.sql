-- Fix PGRST203: resolve ambiguous overload of public.send_personal_voice.
--
-- The live Supabase project has two versions of this function:
--   1) the 6-argument version defined in 003_voice_messages.sql (matches this
--      branch's public.voice_messages table, which has no moderation columns)
--   2) a 9-argument version (adding p_moderation_status, p_moderation_category,
--      p_moderation_reason) applied directly by an unrelated, unmerged branch
--
-- public.voice_messages here has no moderation_* columns, so the 9-argument
-- version cannot be the correct one for this schema. Drop it, and recreate
-- the 6-argument version to guarantee it exactly matches this migration.
