...
## 81.1 Profile Setup（Screen 02）の簡略化
...
また、朝リクエストが未作成の場合でも、81.3で追加した「定期的な起床スケジュール」に登録があれば、その内容を「（初期値）」として表示する。

## 81.3 定期的な起床スケジュール機能（新規追加）
...
## 81.4 Give Choice画面（Screen 06）の廃止
...
- 「誰かに声を届ける」は、Home画面の「明日の誰か」タイムライン（81.2）から直接候補を選ぶ形に統合した。Request List画面自体は、Record画面などの戻り先として引き続き存在する。
